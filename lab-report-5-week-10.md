...
I created a new method `getLinks2()` which is the CommonMark implementation, and kept `getLinks()` the same. I then read the files in both ways, and checked if the two Arraylists were not equal. If they were not, I printed the file's filename and the contents of both results.
